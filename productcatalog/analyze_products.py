...
class ModelAdapter:
    ...
    def _execute_query(self, state: RagState):
        ...
        return {"db_result": self._execute_query_tool.invoke(state["db_query"])}
    ...
    def _merge_smart(self, state:RagState):
        prompt = (
            "Given the following user question, corresponding response generated by db, and that generated from text via RAG, "
            "answer in short the user question as the vendor of products."
            f"Question: {state['question']}\n"
            f"DB response: {state['db_response']}\n"
            f"Text RAG response: {state['rag_response']}"
        )
        response = self.model.invoke(prompt)
    ...
    def _search_db(self, state:RagState):
        ...
        if advanced_mode:
            for step in self.agent.stream( # heavy retrial mechanism
    ...
    def _build_rag_workflow(self):
        graph_builder = StateGraph(self.RagState).add_sequence([self._retrive_docs, self._generate_from_context])
        ...
        graph_builder.add_edge("_search_db", "_build_query")
        graph_builder.add_edge(START, "_search_db")
        graph_builder.add_edge(START, "_retrive_docs")
        graph_builder.add_node(self._merge_smart)
        graph_builder.add_edge("_generate_from_context", "_merge_smart")
        graph_builder.add_edge("_build_answer", "_merge_smart")
